    ...
    def crop_and_convert(
        ...
        for i, (polygon, label, _) in enumerate(polygons_with_labels):
            ...
            for j, (other_polygon, other_label, _) in enumerate(polygons_with_labels):
                ...
                # 检查边界框是否有重叠
                if not (other_x > x_min + width or 
                       x_min > other_x + other_w or 
            ...
            actual_x_min = x_min
            actual_y_min = y_min
            actual_x_max = x_min + width
            actual_y_max = y_min + height
            
            for poly, _ in intersecting_polygons:
                for point in poly:
                    px, py = point
                    actual_x_min = min(actual_x_min, int(px))
                    actual_y_min = min(actual_y_min, int(py))
                    actual_x_max = max(actual_x_max, int(px))
                    actual_y_max = max(actual_y_max, int(py))
            
            # 更新边界框
            x_min = actual_x_min
            y_min = actual_y_min
            width = actual_x_max - actual_x_min + 1
            height = actual_y_max - actual_y_min + 1
            ...
            # 变换所有相交polygon的坐标
            transformed_polygons = []
            for poly, lbl in intersecting_polygons:
                new_polygon = []
                for point in poly:
                    new_x = point[0] - x_min
                    new_y = point[1] - y_min
                    new_polygon.append([new_x, new_y])
                transformed_polygons.append((new_polygon, lbl))
